...
function getEmailProvider() {
    const provider = (0, config_1.getConfig)().email;
    if (!provider) {
        throw new errors_1.ConfigurationError("Email provider not configured. Call configure({ email: ... }) before using email features.");
    }
    return provider;
}
/**
 ...
 * ```
 */
const generateEmail = ({ prefix = "test.user", timestamp = Date.now(), } = {}) => {
 ...
 * ```
 */
async function extractEmailContent({ email, prompt, maxRetries = constants_1.MAX_RETRIES, retryDelayMs = constants_1.EMAIL_RETRY_DELAY, }) {
    const provider = getEmailProvider();
    // Add an initial delay before the first attempt to allow email to arrive
    logger_1.logger.info(`Initial wait before extracting email content for ${email}...`);
    await new Promise((resolve) => setTimeout(resolve, constants_1.EMAIL_INITIAL_WAIT));
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        logger_1.logger.debug(`Waiting for email content (attempt ${attempt}/${maxRetries})...`);
        try {
            const result = await provider.extractContent({ email, prompt });
            logger_1.logger.info(`Successfully extracted email content: ${result}`);
            return result;
        }
        catch (error) {
            logger_1.logger.warn(`Error fetching email content (attempt ${attempt}): ${error}`);
        }
        if (attempt < maxRetries) {
            await new Promise((resolve) => setTimeout(resolve, retryDelayMs));
        }
    }
    throw new errors_1.AIModelError(`Failed to extract email content after ${maxRetries} attempts. Email: ${email}, Prompt: ${prompt}`);
}
